...
using RustExtFetchCallback = void (*)(void *userdata, uint16_t status, const uint8_t *body, size_t body_len,
                                      const char *error);
...
extern "C" uintptr_t rust_ext_emscripten_fetch_start(const char *method, const char *url, const char *const *headers,
                                                     size_t header_count, const uint8_t *body, size_t body_len,
                                                     bool body_present, uint32_t timeout_ms,
	...
	std::strncpy(attributes.requestMethod, stored->method.c_str(), sizeof(attributes.requestMethod) - 1);
